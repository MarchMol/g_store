...
<template>
    <div class="flex gap-4 w-full h-80 overflow-x-auto px-[3rem]">
        <div v-for="p in products" :key="p.id" @click="handleDetail(p.id)"
            class="relative flex flex-col justify-center items-center bg-[var(--color-gray)] min-w-60 max-w-60 min-h-70 max-h-70 p-8 rounded-lg shadow-lg gap-[2rem] cursor-pointer"
        >
            <div @click="handleAdd(p.id)" class="absolute bg-[var(--color-dark-gray)] w-6 h-6 p-2 rounded-4xl flex items-center justify-center top-4 right-4 hover:h-7 hover:w-7 transition-all duration-150 cursor-pointer">
